...
import { loadConfig, RelayConfig, type ApiKeyRecord } from "./config.ts";
import { hashApiKeySecret, createApiKeyAuthenticator } from "./auth/api-key.ts";
...
import type { AuthPrincipal, AgentConnection } from "./schemas.ts";
...
interface RelayServer {
  ...
  config: RelayConfig & { apiKeys: ApiKeyRecord[] };
  start(): Promise<void>;
...
async function createRelayServer(): Promise<RelayServer> {
  ...
  const pendingRequests = new PendingRequestStore(
    config.maxInflightPerAgent,
    config.maxInflightPerOrg,
